          ...
          {activeTab === 'list' && (
            <div className="grid grid-cols-1 gap-4 animate-fadeIn">
              {policies.length === 0 ? (<div className="p-8 text-center bg-white rounded-2xl border border-slate-200 text-slate-500 font-medium">No policies found. Click "+ Add New Policy" to get started!</div>) : (
                policies.map(p => (<div key={p._id} className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition"><div className="flex justify-between items-start mb-2"><h4 className="font-bold text-lg text-slate-800">{p.title}</h4><div className="flex gap-2"><button onClick={() => handleEdit(p)} className="text-sm font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition">Edit</button><button onClick={() => handleDelete(p._id)} className="text-sm font-bold text-red-600 bg-red-50 px-3 py-1.5 rounded-lg hover:bg-red-100 transition">Delete</button></div></div><p className="text-xs text-slate-400 mb-4 font-mono">Last updated: {new Date(p.updatedAt).toLocaleString()}</p><p className="text-slate-600 whitespace-pre-wrap text-sm leading-relaxed">{p.description}</p></div>))
              )}
            </div>
          )}
        </div>
