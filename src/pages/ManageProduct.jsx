...
const ManageProduct = ({ token, stores, onLogout }) => {
  ...
  const handleImportDefaultProducts = async () => {
    const remainingAllowed = maxProducts - products.length;
    if (maxProducts > 0 && selectedDefaultProducts.length > remainingAllowed) {
      alert(`Plan limit exceeded. You can only import up to ${remainingAllowed} more products. Please upgrade your plan or select fewer items.`);
      return;
    }
  ...
  const activePlanId = currentStore.planId && typeof currentStore.planId === 'object' ? currentStore.planId._id : currentStore.planId;
  const activePlan = plans.find(p => p._id === activePlanId) || (currentStore.planId && typeof currentStore.planId === 'object' ? currentStore.planId : null) || plans[0] || {};
  const maxProducts = activePlan?.limits?.maxProducts || 0;
        ...
        {displayedProducts.length > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between px-6 py-4 border-t border-slate-200 bg-slate-50 gap-4">
            <div className="text-sm text-slate-500 flex items-center gap-2">
              Showing <span className="font-bold text-slate-800">{startIndex + 1}</span> to <span className="font-bold text-slate-800">{Math.min(startIndex + itemsPerPage, displayedProducts.length)}</span> of <span className="font-bold text-slate-800">{displayedProducts.length}</span> products
              <select 
                value={itemsPerPage} 
